...
九路真实硬件、300 样本的新训练入口见 [硬件数据优化说明](HARDWARE300.md)。该流程直接分类实测电流，并与下述模拟 RC 实验分别报告。

后续优化增加响应分布统计、L-BFGS 神经网络及多模型融合，详见 [第二轮复现说明](HARDWARE_REFINEMENT.md) 和 [完整对比结果](../results/rc10_hardware300_v2/README.md)。这是同一批数据上的自适应探索，无独立测试集。

第三轮在 `feat/hardware300-v3` 中冻结 42 个纯电流候选并完成三组五折嵌套评估，入口和复现命令见 [第三轮说明](HARDWARE_REFINEMENT_V3.md)，聚合结果见 [v3 结果](../results/rc10_hardware300_v3/README.md)。v3 平均准确率 88.33%，低于第二轮 90.89%，因此保留 v2 作为推荐方案。
