...
| RC10 九路真实硬件，300 样本嵌套验证均值 | 83.44% | 83.44% | 三次五折；准确率标准差 1.64 个百分点，无独立测试集 |
| RC10 九路真实硬件，第二轮探索均值 | 90.67% | 90.67% | 响应分布＋神经网络／树融合；无独立测试集 |
| RC10 九路真实硬件，初始化扩展探索均值 | 90.89% | 90.89% | 扩展至 54 候选，未改善划分间波动；无独立测试集 |

RC10 是筛选后的类别，不能与完整 38 分类直接横向比较；两条路线的验证划分与选择流程也不同。ConvSNN 硬件感知结果不代表真实芯片部署测量。九路真实硬件实验只使用收到的实测电流，与模拟 RC10 的 97.02% 不属于同一实验；第二轮从 83.44% 提升至 90.67%，初始化扩展为 90.89%，但与历史探索记录 90.44% 仅差 0.44 个百分点，尚不能证明稳定超过历史方案。第二轮开发筛选未完全嵌套，不能作为无偏泛化估计。精确指标及来源见 [results](results/README.md)。
