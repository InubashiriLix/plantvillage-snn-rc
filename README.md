...
RC10 是筛选后的类别，不能与完整 38 分类直接横向比较；两条路线的验证划分与选择流程也不同。ConvSNN 硬件感知结果不代表真实芯片部署测量。九路真实硬件实验只使用收到的实测电流，与模拟 RC10 的 97.02% 不属于同一实验；本轮 83.44% 未超过历史探索记录的 90.44%，不建议据此替换原方案。精确指标及来源见 [results](results/README.md)。
